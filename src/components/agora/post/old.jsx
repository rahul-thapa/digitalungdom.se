...
import {
  Avatar, Dropdown, Divider, Empty, Row, Col, Card, Menu, Skeleton, Tag,
} from 'antd';
  ...
  <Menu>
    <Menu.Item>
      <span onClick={() => copyPostLinkToClipBoard(link)}>
        <FontAwesomeIcon style={{ marginRight: 4 }} icon={faCopy} />
        {' '}
Kopiera länk
      </span>
    </Menu.Item>
  ...
  return (
    <span>
      <span onClick={() => showModal(true)}>
        <FontAwesomeIcon style={{ marginRight: 4 }} icon={faFlag} />
        {' '}
Anmäl inlägg
      </span>
      <Modal
...
const editMenu = (antiAgorize, deleteTemp) => (
  <Menu>
    <Menu.Item>
      ...
      >
        <FontAwesomeIcon style={{ marginRight: 4 }} icon={faTrash} />
        {' '}
Radera inlägg
      </span>
    </Menu.Item>
...
function Post({
  ...
  const wordLimitFadedDisplay = 500;
  ...
  const Body = () => {
    ...
    if (post.body) {
      ...
      if (post.type === 'link') {
        return (
          <p>
            <a href={(post.body.indexOf('https://') === -1 && post.body.indexOf('http://') === -1) ? `https://${post.body}` : post.body} rel="noopener noreferrer" style={{ fontSize: 16 }} target="_blank">
              {' '}
              {post.body}
              {' '}
            </a>
                      ...
                      showComments
                        ...
                        : (
                          <div>
                            ...
                            <div style={{
                              position: 'absolute', bottom: 0, height: 100, width: '100%', backgroundImage: 'linear-gradient(rgba(255,255,255,0), rgba(255,255,255,1))',
                            }}
                            />
                          </div>
                        )
            ...
            <Row style={{ fontSize: 16, marginBottom: 10 }}>
              ...
              >
                <div className="WillNotOpenPostDiv" style={{ width: '100%', height: '100%', paddingTop: 2 }}>
                  <FontAwesomeIcon style={{ marginRight: 4 }} color={isStarClicked ? 'gold' : ''} icon={faStar} />
                  {' '}
                  {post.stars}
                </div>
              </Col>
